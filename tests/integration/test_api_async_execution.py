...
async def test_async_vertical_slice(
    ...
    # never part of the public timeline. The vertical slice runs the legacy
    # Google Public DNS provider, which fails closed under PR 28B (the
    # documented not-semantically-modeled boundary), so no analysis round is
    # requested and no assessment is produced.
    assert InvestigationTimelineEventType.INVESTIGATION_STOPPED.value in event_types
    assert InvestigationTimelineEventType.PROVIDER_WORK_FAILED.value in event_types
    assert InvestigationTimelineEventType.ASSESSMENT_REQUESTED.value not in event_types
