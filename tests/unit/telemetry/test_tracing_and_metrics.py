...
class TestSpanVocabulary:
    ...
    def test_exact_canonical_span_set(self) -> None:
        ...
        assert {
            ...
            "ati.llm.invoke",
            "ati.report.generate",
            "ati.provider.execute",
            "ati.embedding.invoke",
...
class TestMetricVocabulary:
    ...
    def test_exact_canonical_metric_names(self) -> None:
        ...
        assert {spec.name for spec in COUNTER_SPECS} == {
            ...
            "ati.datasource.acquire.failures",
            "ati.datasource.convert.failures",
            "ati.provider.http.attempts",
            "ati.provider.http.retries",
            "ati.provider.http.failures",
            "ati.provider.execute.failures",
            "ati.geo.resolve.resolved",
            "ati.geo.resolve.unresolvable",
            "ati.geo.resolve.failed",
            "ati.investigation.execute.executed",
            "ati.investigation.execute.failures",
            "ati.agent.invoke.failures",
    ...
    def test_duration_metric_constants_use_seconds_suffix(self) -> None:
        ...
        for name in (
            ...
            DurationMetrics.DATASOURCE_ACQUIRE,
            DurationMetrics.DATASOURCE_CONVERT,
            DurationMetrics.PROVIDER_HTTP,
            DurationMetrics.PROVIDER_EXECUTE,
            DurationMetrics.GEO_RESOLVE,
            DurationMetrics.INVESTIGATION_EXECUTE,
            DurationMetrics.AGENT_INVOKE,
