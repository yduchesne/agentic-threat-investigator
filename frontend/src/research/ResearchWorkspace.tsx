...
// Research workspace content (PR 24C §10; PR 24D §6, §10).
//
...
export function researchColumns(t: (key: string) => string): Column<ResearchResult>[] {
  return [
    {
      id: "subjectEntity",
      ...
      render: (research) => (
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          <CompactId id={research.subject_entity_id} label={t("columns.subjectEntity")} />
          <PivotMenu
            actions={researchSubjectActions(research, "table_cell")}
            ariaLabel={t("columns.subjectEntity")}
          />
        </Box>
      ),
...
export interface ResearchWorkspaceProps {
  ...
  table: ResourceTableState<ResearchFilters>;
  /** Rendered inside the pivot modal: the modal chrome owns title/nav. */
  embedded?: boolean;
}

/** The Research workspace content shared by the route and the modal. */
export function ResearchWorkspace({
  investigationId,
  investigation,
  table,
  embedded = false,
}: ResearchWorkspaceProps): ReactElement {
