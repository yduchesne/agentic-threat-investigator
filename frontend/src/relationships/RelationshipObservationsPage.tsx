...
// First-class RelationshipObservation route (PR 24C §9).
//
// /investigations/:id/relationships/observations
//
// RelationshipObservation is the immutable historical observation and is
// browsed directly — never routed through generic History. ``observed_at``
// and ``retrieved_at`` remain visibly independent with half-open ranges;
// no started/ended/removed/continuous-validity semantics are inferred.
// Detail uses the exact list DTO (no single-GET endpoint exists and none
// is invented).
...
function observationColumns(t: (key: string) => string): Column<RelationshipObservation>[] {
  return [
    {
      id: "relationship",
      header: t("columns.relationship"),
      render: (observation) => <CompactId id={observation.relationship_id} label={t("columns.relationship")} />,
    ...
    },
    {
      ...
      render: (observation) => <CompactId id={observation.evidence_id} label={t("columns.evidence")} />,
