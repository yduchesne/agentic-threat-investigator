...
- Every request traverses a real `httpx.AsyncClient` over `httpx.ASGITransport` into an ATI-authored FastAPI stub application. Virtual upstreams (Google DNS, IANA bootstrap, authoritative RDAP services, ThreatFox) are dispatched by request host and path inside genuine ASGI routes; tests assert on request state recorded by the routes, proving requests reached the ASGI application rather than a direct mock callback.
...
- Routes assert exact method, `User-Agent`, and `Accept` headers, the DNS route asserts the absence of EDNS client-subnet parameters, and the ThreatFox route asserts the exact POST search body, `Auth-Key` header, and JSON content type while proving the credential never appears in the URL or body. All payloads are synthetic, local, and credential-free.
