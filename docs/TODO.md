...
## PR 19B secondary provider-execution and timeline hardening

**Priority:** MEDIUM

**Origin:** PR 19B branch review against `.plans/PR_19B_LUNA_EXECUTION_PLAN.md`.

### Problem

The CRITICAL/HIGH remediation is specified only in
`.plans/pr-19b-fixes-01.md`. The following lower-severity follow-up remains:

- `ProviderComposition.provider_registry()` and `ProviderWorkExecutor` use
  `Mapping[str, EvidenceProvider]` rather than the plan's stronger
  `Mapping[SourceId, EvidenceProvider]` shape.
- Outcome and aggregate timeline identifier lists can contain duplicates when
  several Evidence observations resolve the same Entity or Relationship. State
  bookkeeping deduplicates them later, but the outcome/timeline representation
  is less canonical than the state.
- `InvestigationTimelineEvent.error_code` strips padding and has no lexical or
  length bound, and the model does not enforce event-type-specific field
  combinations. The executor emits safe values, but direct model/repository
  callers can construct semantically odd events.
- Timeline append-only behavior is proven at the repository API surface only;
  the integration test does not attempt a direct database UPDATE/DELETE or
  document the database-role protection boundary.
- Migration 0013 is checked as part of head migration and table discovery, but
  has no focused `0012 -> 0013 -> 0012 -> 0013` lifecycle test. The timeline
  sequence is also not declared `OWNED BY` its table column.
- `PostgresUnitOfWork.__aexit__()` does not clear `timeline_events` after closing
  the session, unlike the other repository attributes.
- The production executor/registry are instantiated by integration tests, but
  there is no small public composition factory or package export assembling
  `ProviderWorkExecutor` with the graph for a future worker entry point.
- The DNS vertical-slice seed helper initially persists a random
  `root_entity_ids` value and only later creates the actual DOMAIN Entity. The
  test constructs a corrected in-memory state afterward, so the provider
  pipeline passes, but the durable Investigation row does not identify the
  Entity described as its root.
- Timeline FK rejection is asserted with a broad `Exception`, which does not
  prove the expected PostgreSQL foreign-key SQLSTATE.
- The HIGH-4 timeline logging regression declares a synthetic secret marker,
  but `FakeTimelineSink` raises its fixed `"injected timeline failure"` text;
  the marker is never placed in the caught exception. The production logger
  now omits exception details, but this test does not prove that path with the
  adversarial marker it asserts.
- `test_provider_executor_regressions.py` imports numerous private fixtures and
  constants from another test module and uses broad module-level Pylint
  suppressions. This passes today but makes test collection/refactoring more
  fragile than a shared typed support fixture.
- The fixes-02 noncanonical/malformed subject tests assert zero persistence but
  use the default no-op extractor without recording or exploding. The public
  executor currently validates before extraction, but those individual tests
  do not directly prove the required zero-extraction property.
- `_target_binding_error()` runs once before provider invocation and then again
  inside `_binding_violation()` after the provider returns. The second check is
  defensive but redundant for the immutable local `target` and leaves
  unreachable/duplicate validation branches in an already return-heavy path.

### Intended fix

1. Type provider registries with `SourceId` keys end to end, or document why
   source URN strings are the established composition key. Keep unknown/
   unconfigured provider behavior unchanged.
2. Deduplicate evidence/entity/relationship IDs in first-seen order before
   building outcomes and aggregate timeline events. Add a repeated-stable-
   identity test; do not change persistence semantics.
3. Add conservative `error_code` length/grammar validation and event-type field
   validation only after documenting the exact contract. Keep events free of
   prose and do not add PR 20/21 reasoning fields.
4. Add a focused database immutability test or document that append-only is
   enforced by application roles/API rather than a trigger. Do not introduce a
   row-level domain-versioning trigger.
5. Add isolated migration 0013 downgrade/re-upgrade coverage and make the
   sequence owned by `investigation_timeline_event.sequence` if compatible with
   downgrade behavior.
6. Reset `timeline_events` to the same inactive sentinel in
   `PostgresUnitOfWork.__aexit__()` as every other repository attribute; add a
   lifecycle test.
7. When a worker/bootstrap entry point exists, expose one typed composition
   seam that assembles the provider registry, entity reader, extractor, PR 18C
   service, timeline sink, executor, and existing graph. Do not add a worker in
   this follow-up.
8. Seed the actual root Entity UUID into the persisted Investigation in the DNS
   integration fixture, then assert the durable `root_entity_ids` contains that
   exact ID.
9. Assert the exact foreign-key exception/SQLSTATE for an unknown timeline
   investigation and verify no row remains after rollback.
10. Make the timeline logging fake raise an exception whose message actually
    contains the synthetic marker, then assert both `caplog.text` and every
    captured record's message exclude it while retaining the stable
    `timeline_error` category.
11. Move shared provider-executor fakes/constants into a typed
    `tests/support` module (or keep related tests in one module) and narrow
    lint suppressions to the smallest justified scope.
12. Give each noncanonical/malformed binding regression an extractor that
    records calls or raises immediately, then assert it was never invoked in
    addition to asserting zero PR 18C calls.
13. Keep target validation before provider invocation as authoritative. Remove
    the repeated target validation inside `_binding_violation()`, or document a
    concrete mutation/concurrency reason for retaining it; preserve all binding
    behavior and tests.

### Acceptance checks

- Registry key typing is explicit and consistent.
- Outcome/timeline identifiers are canonical first-seen lists.
- Timeline model validation remains bounded and reasoning-free.
- Append-only enforcement and migration lifecycle are tested accurately.
- A closed UoW exposes no stale timeline repository.
- The durable vertical-slice Investigation references the actual root Entity.
- `./build.sh --qa` and `./integration-test.sh` pass.
