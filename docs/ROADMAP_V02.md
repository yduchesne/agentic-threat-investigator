...
### PR 28H — End-to-end closure **`[DONE]`**
...
**Delivered:** the real-stack distributed Evidence ingestion closure in `tests/integration/kafka/test_distributed_evidence_ingestion.py` (H28H-01..21). Deterministic ThreatFox-format fixture bytes traverse the production path end to end: real `ProviderHttpClient` boundary (the only faked surface is the external Internet endpoint), real `ThreatFoxDatasource` acquirer/parser, real `ThreatFoxToEvidenceConverter`, `DatasourceEvidenceProducer`, `EvidenceMessage` V1 with the PR 28C codec, real `KafkaEvidencePublisher`, real Redpanda, real `KafkaEvidenceConsumer`, `EvidencePersistenceConsumer`, real `EvidenceBatchPersistenceService`, and real PostgreSQL. The matrix proves fresh-A creation, later A-equivalent persistence as `UNCHANGED` (never exact replay), A->B, A->B->C, A->B->A recurrence (a distinct message appends v3 — never suppressed as replay), one atomic multi-Evidence batch, exact-message replay as a receipt no-op, same-semantic-new-execution as a new receipt with unchanged EO history, failure before PostgreSQL commit (no partial state, no Kafka commit, redelivery, one retry), PostgreSQL-commit-then-Kafka-commit-absent recovery through the at-least-once window, poll-then-stop redelivery, atomic multi-record rollback with an uncommitted broker, successful-commit restart with no redelivery, producer `COMPLETED` without any consumer, `publication_failed` on publish failure, post-terminal lifecycle append rejection, exact `EvidenceObservationEntity` and `RelationshipObservation` provenance per committed observation, and explicit observation-exact Investigation admission across two Investigations, v1+v2, and later-global-no-mutation scenarios. DB-commit-before-offset-commit, at-least-once replay safety, transport-only positions, and unchanged PR 28E receipt semantics are asserted against authoritative PostgreSQL rows. No database migration, no Kafka transactions, no retry topics/DLQ, no Schema Registry, no new message version, no Investigation admission from ingestion, and no generalized worker/scheduler framework were added; production runtime wiring keeps the transitional synchronous Investigation path documented in `DATASOURCE_ARCHITECTURE.md` (replacing it would require Investigation orchestration redesign).
