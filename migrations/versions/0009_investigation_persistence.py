...
def upgrade() -> None:
    ...
    sql_path = Path(__file__).parents[1] / "sql/ati/v0006/investigation_persistence.sql"
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP FUNCTION IF EXISTS ati.append_evidence(uuid,uuid,text,uuid,text,text,text,timestamptz,timestamptz,jsonb,jsonb,uuid,uuid)"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS ati.soft_delete_investigation(uuid,uuid,uuid,bigint)"
    )
    op.execute(
