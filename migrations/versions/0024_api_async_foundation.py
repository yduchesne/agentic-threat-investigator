...
def upgrade() -> None:
    ...
    op.execute(
        Path(__file__).parents[1].joinpath("sql/ati/v0020/api_async.sql").read_text()
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP FUNCTION IF EXISTS ati.create_investigation_job(uuid, timestamptz)"
    )
    op.execute("DROP FUNCTION IF EXISTS ati.claim_next_investigation_job(timestamptz)")
    op.execute(
