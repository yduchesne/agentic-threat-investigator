-- Immutable SQL API v0009 for PR 18C graph integrity.
--
-- Remediation of v0008 review findings:
-- - ati.upsert_entity rejects a soft-deleted canonical identity with a
--   dedicated SQLSTATE while holding the authoritative row lock, so a
--   concurrent soft deletion can never be bypassed by a pre-lock read;
-- - ati.soft_delete_relationship makes Relationship soft deletion a
--   database-owned write: locked validation, dedicated SQLSTATEs for
--   absent/already-deleted and stale expected versions, sequence-allocated
--   version, and exactly one immutable DELETE history row with the canonical
--   JSONB diff.
-- The relationship_observation Evidence foreign key is plain DDL and lives in
-- the Alembic migration. Row-level triggers are never used for versioning or
-- history.
...
BEGIN
  ...
  END IF;
  -- Fail-closed PR 18C policy: the row lock is authoritative, so a soft
  -- deletion committed after the caller's pre-lock read is still rejected
  -- before any version allocation, metadata update, or history write.
  IF (old_state->>'deleted_at') IS NOT NULL THEN
    RAISE EXCEPTION 'soft-deleted entity rediscovered' USING ERRCODE = 'U18C2';
  END IF;
...
CREATE OR REPLACE FUNCTION ati.soft_delete_relationship(
...
BEGIN
  -- Order while holding the row lock: load, absent/already-deleted, stale
  -- expected version, and only then mutation and immutable DELETE history.
  SELECT to_jsonb(r), r.version INTO old_state, current_version
    FROM ati.relationship r WHERE r.id = p_id AND r.deleted_at IS NULL FOR UPDATE;
  IF old_state IS NULL THEN
    RAISE EXCEPTION 'relationship not found or already deleted'
      USING ERRCODE = 'U18C3';
  END IF;
  IF p_expected_version IS NOT NULL AND current_version <> p_expected_version THEN
    RAISE EXCEPTION 'optimistic version conflict' USING ERRCODE = 'U18A2';
  END IF;
  UPDATE ati.relationship AS rel SET deleted_at=now(), deleted_by_actor_id=p_actor_id,
    version=nextval('ati.relationship_version_seq'), updated_at=now()
    WHERE rel.id=p_id
    RETURNING rel.id, rel.version, to_jsonb(rel)
    INTO id, version, new_state;
  INSERT INTO ati.domain_object_history(
    object_type, object_id, version, operation, state, diff)
    VALUES('relationship', id, version, 'DELETE', new_state,
           ati.ati_jsonb_diff(old_state, new_state));
  RETURN NEXT;
END $$;
